...
class Version < JsDuck::Tag::Tag
  ...
  def parse_doc(scanner, position)
    text = scanner.match(/.*$/)
    return { :tagname => :version, :text => text }
  end

  def process_doc(context, platform_tags, position)
    context[:version] = platform_tags.map {|tag| tag[:text] }
  end

  def to_html(context)
    versions = context[:version].map {|version| "<b>#{version}</b>" }.join(" and ")
    <<-EOHTML
